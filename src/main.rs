	...
	fn render(&mut self) {
		...
		{ // stock_market rendering
			...
			if self.state.is_specific_stock_open {
				...
				let price_current_str = price_current.to_string_pretty(2); // TODO?: 3
				{ // render top text
				...
				{ // plot prices over time
					...
					let price_gmin_str = price_gmin.to_string_pretty(2); // TODO?: 3
					let price_gmax_str = price_gmax.to_string_pretty(2); // TODO?: 3
					...
					let price_min_str = price_min.to_string_pretty(2); // TODO?: 3
					let price_max_str = price_max.to_string_pretty(2); // TODO?: 3
				...
				while i - i_init < ITEMS_N && i < self.state.stock_market.stocks.len() as u32 {
					...
					{ // render text
						...
						let price_str = price.to_string_pretty(2); // TODO?: 3
...

		{
			...
			let mut top_left_lines = vec![
				(format!("$: {} + {}", self.state.money.to_string_pretty(2), self.state.stock_market.calc_money_in_stocks().to_string_pretty(2)), self.state.is_stock_market_open.select(ColorU8::WHITE, color)), // TODO?: 3
